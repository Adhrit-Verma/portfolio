...
import { archive, files } from "@/lib/resume";
import { ScrollTrigger, glitchCut, scan, useGsapScope, useReducedMotion } from "@/lib/motion";
...
function FileCard({ file }: { file: FileRecord }) {
  ...
  return (
    ...
    >
      ...
      <ul data-stagger className="mt-7 flex flex-wrap gap-1.5">
        {file.stack.map((s) => (
          <li key={s}>
  ...
  return (
    <Chapter id="files" className="overflow-hidden">
      ...
      <ChapterHeader
        ...
        label="Files"
        title="Hacked Files"
        meta={`${files.length} FULL · ${archive.length} PARTIAL`}
      />
      ...
      {/* Deliberately thin: no verified detail exists for these yet. */}
      <div className="mt-14">
        <div className="mb-5 flex items-center gap-3 border-b border-line pb-3">
          <Pill tone="dim">PARTIAL RECOVERY</Pill>
          <p className="chrome text-dim">Fragments — metadata only</p>
        </div>
        <ul className="grid gap-px bg-line sm:grid-cols-2 lg:grid-cols-3">
          {archive.map((a) => (
            <li key={a.code} className="bg-void p-5 transition-colors hover:bg-panel-hi/70">
              <div className="flex items-center justify-between">
                <span className="chrome text-scan">{a.code}</span>
                <span className="chrome text-dim/60">34%</span>
              </div>
              <h3 className="mt-3 text-base font-semibold uppercase tracking-wide text-ink">
                {a.name}
              </h3>
              <p className="mt-2 text-sm leading-relaxed text-dim">{a.hook}</p>
              <div aria-hidden="true" className="mt-4 h-px w-full bg-line">
                <div className="h-px w-[34%] bg-signal/60" />
              </div>
            </li>
          ))}
        </ul>
